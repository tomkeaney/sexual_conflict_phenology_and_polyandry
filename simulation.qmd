...
For males, reproductive success depends on mating success, the fecundity of a male's mates and the intensity of sperm competition the male faces within the reproductive tract of each female he inseminates. We model sperm competition under two different scenarios: 1) sperm-dumping, where a single mating is sufficient to fill the spermatheca. Thus, when a female mates with a second male, she dumps half of the total sperm she has received randomly (as she has received enough to fill her spermatheca twice-over), such that her spermatheca now contains equal parts of male one and male two's ejaculate. If she mates a third time, she again dumps half the sperm, such that male three contributes half the present sperm, and male one and male two's representation is reduced to a quarter each. A female's recent mates therefore have the highest chance of siring her offspring. To put this in perspective, a male's probability of fathering offspring from a given reproductive bout reduces to ~0.01 if the female has subsequently mated six more times. 2) complete sperm-mixing without any sperm dumping. Here, a female's spermatheca expands to store all the sperm she receives across her life. Paternity is evenly divided between her mates i.e. an egg of a thrice-mated female has a 33% chance of being fertilised by a sperm from each male. In both cases, I've made the simplifying assumption that fertilisation and sperm mortality have negligible effects on the number of sperm in the spermatheca.  For completeness, we consider cases where females have mated up to 15 times, though this never occurs in the currently run simulations.
...
main_sim <- function(row, # which row of the parameter space
  ...
  mean_trait_value <- matrix(NaN, ncol = 5, nrow = length(0:gens))
  ...
  while(G <= gens & matings > 0){  
    ...
    mean_trait_value[G+1,5] <- 1 - sum(!is.na(female_mating_partners[,1]))/(popsize - sum(pop[,1]))
...
if(file.exists("results/single_example_sperm_precedence.rds")){test_1 <- read_rds("results/single_example_sperm_precedence.rds")
}else{test_1 <-
  main_sim(row = 1,
...
write_rds(test_1, "results/single_example_sperm_precedence.rds")
}
# fair raffle
if(file.exists("results/single_example_fair_raffle.rds")){test_2 <- read_rds("results/single_example_fair_raffle.rds")
}else{test_2 <-
  main_sim(row = 1,
           input = parameters_single_example,
           sex_limited = 1,
...
write_rds(test_2, "results/single_example_fair_raffle.rds")
}
...
p1<-
  ...
  labs(y = "Mean emergence time (breeding value)",
       colour = "Sex",
       title = "Last male sperm precedence") +
  theme_bw() +
...
p2<-
  ...
  labs(y = "Mean emergence time (breeding value)",
       colour = "Sex",
       title = "Fair raffle sperm competition") +
  theme_bw() +
  theme(legend.position = "bottom",
        plot.title = element_text(hjust = 0.5))
...
top <- (p1 + p2) + plot_layout(axis_titles = "collect", guides = "collect") & 
  theme(legend.position = 'bottom')

bottom <- (p3+p4) + plot_layout(axis_titles = "collect")
...
OSR_f_1 + OSR_m_1 + OSR_1 + guide_area() +
  plot_layout(guides = 'collect')
  
```

## Fair raffle sperm competition
...
OSR_f_2 + OSR_m_2 + OSR_2 + guide_area() +
  plot_layout(guides = 'collect')
